...
def get_data_from_csv(columns, countries=None, start=None, end=None):
    ...
    if countries is None:
        ...
        countries = list(df_latest_dates.nlargest(6, columns[0])["location"])
        print(countries)
...
def plot_reported_cases_per_million(countries=None, start=None, end=None):
    ...
    cases_df.dropna(subset = ["new_cases_smoothed_per_million"], inplace=True)
    ...
    return chart.configure_title(fontSize=24)
...
def main():
    ...
    """
    # countries = ['Germany', 'Australia', 'Brazil', 'England', 'Norway', 'Taiwan']
    start = '2020-02-26'
    end = '2021-11-20'
    chart = plot_reported_cases_per_million(countries=None, start=start, end=end)
